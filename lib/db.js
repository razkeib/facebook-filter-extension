...
export async function savePost(newPost) {
  if (!newPost || !newPost.postId) return { status: 'skipped' };
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      getReq.onsuccess = () => {
        ...
        let action = 'inserted'; // Default action

        if (existing) {
          action = 'updated'; // We found an existing record!
          ...
          postToSave = {
            ...existing,
            ...newPost,
            capturedAt: existing.capturedAt || Date.now(), // Preserve original capture timestamp
          ...
          // New post insertion: stamp the exact capture time
          postToSave = {
            ...newPost,
            capturedAt: Date.now()
          };
        ...
        // Return the action status along with the post data
        putReq.onsuccess = () => resolve({ status: action, post: postToSave });
...
export async function clearAllData() {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`[FB DB] 🗑️ All data was CLEARED`);
    });
  } catch (err) {
