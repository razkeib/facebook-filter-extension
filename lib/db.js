...
export async function updatePostFlags(postId, updates) {
  if (!postId) return;
  try {
    const db = await getDB();
    return new Promise((resolve, reject) => {
      ...
      getReq.onsuccess = () => {
        ...
        const putReq = store.put(updatedPost);

        putReq.onsuccess = () => resolve(updatedPost);
        putReq.onerror = () => reject(putReq.error);
      };
    ...
    console.warn("[FB DB] Error updating post flags:", err?.message || err);
  }
}
