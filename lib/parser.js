...
export function parseGraphQLPayload(rawBody) {
  ...
  // Clean Facebook JS security prefixes if present (e.g. "for (;;);")
  ...
  for (const line of lines) {
    ...
    try {
      ...
      // Collect top-level prefetch URIs from payload extensions if present
      const prefetchUris = [];
      if (payload.extensions && Array.isArray(payload.extensions.prefetch_uris_v2)) {
        payload.extensions.prefetch_uris_v2.forEach(item => {
          if (item && item.uri) prefetchUris.push(item.uri);
        });
      }
      ...
      // Distribute top-level prefetch URIs to posts harvested in this chunk
      ...
      // Skip non-JSON or chunk fragment lines
    }
  }
...
/**
 * Recursively locates valid post nodes inside arbitrary GraphQL responses.
 */
function findStoryNodes(obj, results, seen) {
  ...
  if (isGraphQLStory || hasStoryIdentifiers) {
    ...
    if (postId && !seen.has(postId)) {
      ...
      // Only add node if it contains usable post content
      if (text || images.length > 0 || author.name !== "Unknown Author") {
        ...
        return; // Don't recurse deeper into the same story object once processed
      }
    }
  }

  // Recurse down into child objects and arrays
...
/**
 * Recursively scans object node to find author information.
 */
...
/**
 * Extracts target Group metadata if the post was made inside a group.
 */
function extractGroup(node) {
  if (!node || typeof node !== 'object') return { id: null, name: "Facebook Feed" };
  ...
  const traverse = (obj) => {
    ...
    if (obj.to && typeof obj.to === 'object' && (obj.to.name || obj.to.id)) {
      groupData = { id: obj.to.id || null, name: obj.to.name || "Facebook Group" };
      return;
    }
  ...
  traverse(node);
  return groupData || { id: null, name: "Facebook Feed" };
}

/**
 * Recursively extracts post text from main bodies, comet sections, or attached stories.
 */
...
/**
 * Extracts image URIs from media structures, attachments, and photos.
 */
function extractImages(node) {
  ...
  const traverse = (obj) => {
    ...
    if (obj.photo_image && typeof obj.photo_image.uri === 'string') {
